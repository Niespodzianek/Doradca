...
def logika_srednich(dluzsze_srednie, dluzsza, krotsze_srednie, krotsza):
    if len(dluzsze_srednie) > 2 and len(krotsze_srednie) > 2:
        if (
            ...
            print(
                f"Krótsza średnia {krotsza} przebiła od góry średnią {dluzsza}. SYGNAŁ ZMIANY DŁUGOTERMINOWEGO TRENDU "
                f"NA SPADKOWY"
            )
        elif (
            dluzsze_srednie[-1] < krotsze_srednie[-1]
            ...
            print(
                f"Krótsza średnia {krotsza} przebiła od dołu średnią {dluzsza}. SYGNAŁ ZMIANY DŁUGOTERMINOWEGO TRENDU "
                f"NA ROSNĄCY"
            )
        elif (
            dluzsze_srednie[-1] < krotsze_srednie[-1]
...
def program():
    ...
    while program_pracuje:
        ...
        if srednie_sma_8 and srednie_sma_21:
            print(
                f"Kurs - otwarcie: {notowania[0]:.2f}, maks: {max(notowania):.2f}, min: {min(notowania):.2f}, ostatni:"
                f" {notowania[-1]:.2f}, średnia SMA z 8 sesji: {srednie_sma_8[-1]:.2f} a z 21 sesji:"
                f" {srednie_sma_21[-1]:.2f}"
            )
        elif srednie_sma_8:
            print(
                f"Kurs - otwarcie: {notowania[0]:.2f}, maks: {max(notowania):.2f}, min: {min(notowania):.2f}, ostatni:"
            ...
            print(
                f"Kurs - otwarcie: {notowania[0]:.2f}, maks: {max(notowania):.2f}, min: {min(notowania):.2f},"
...
#  TODO 1. średnie SMA 8, 21, ROC, RSI, linie Boolingera
#  TODO 2. wykresy matplotlib
